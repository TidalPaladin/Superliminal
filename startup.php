...
function display_network() {
	$connection_status = nmcli_get_status('connected', true)[0];
	// print_r($connection_status['signal']);
	if( !empty($connection_status) )
		return $connection_status['signal'].'%';
}
?> 
...
<div id='dropbox' class='info' style='display: none;'>
	...
	<div class='console' style=''>
		<table id='startup_info' style='width:100%'>
			<thead>
				<H2>Account Info</H2>
			</thead>
			<tbody>
				<tr>
					<td>Account:</td>
					<td id='account' class='startup_info_field'></td>
				</tr>
				<tr>
					<td>Email:</td>
					<td id='email' class='startup_info_field'></td>
				</tr>
				<tr>
					<td>Local IP:</td>
					<td id='ip' class='startup_info_field'><?php echo rtrim(shell_exec("ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1'"))?></td>
				</tr>
				<tr>
					<td>Signal:</td>
					<td id='ip' class='startup_info_field'><?php echo display_network()?></td>
				</tr>
			</tbody>
		</table>
	</div>

...
//Set mode if coming from setup wizard, change Apply button to move to next step (JS)
...
// Process nmcli
//$devices = nmcli_get_networks();

// Get list of active networks
$connection_status = nmcli_get_status('connected');
?>

<script>

function wizard_changes() {
	...
	<tr>
		<td>
			...
			<thead>
				...
				<tr>
					<td colspan='2'><H4 class='table_heading' style='margin-top: 10px'>Active Connections</h4></td>
				</tr>
				<?php
					if( $connection_status )
						foreach($connection_status as $key => $value)
							echo '<tr><td colspan="2"><p><span style="color: green; font-weight:bold">Connected</span> to <span style="font-weight:bold">'.$value['con'].'</span> with IP '.$value['ip'].'</p></td></tr>';
					else
						echo '<tr><td colspan="2"><p><span style="color: red">Disconnected</span> - There are no active connections</p></td></tr>';
				?>
				<tr>
					<td><h4 class='table_heading'>Add Network</h4></td>
				</tr>
			</thead>
			<tbody>
			<form action="#" onsubmit="return validateForm( $(this) )" method="POST" id="connect">
				...
				<tr>
					...
					<td><?php 
						...
						generateNetworksDropdown(  nmcli_get_networks('--'), 'networks_forget' );?></td>
				</tr>
				<tr>
					<td>Join Network:</td>
		...
		<table>
			<tr>
				<td valign='top'>
					<table>
						...
						<tbody id="scan_results" style='display:none'>
							<tr>
			...
			<tr id='known_networks' style='text-align:center; display:none'>
				<td>
					<table style='width:100%'>
						<thead>
							<tr>
								<td colspan="2"><H2 style='text-align:center; margin-top:20px'>Known Networks</H2></td>
							</tr>
						</thead>
						<tbody style='text-align:center'>
							<?php
								$networks = nmcli_get_networks();
								foreach( $networks as $key => $value ) {
									echo '<tr><td colspan="2"><p>'.$value['name'].'</p></td></tr>';
								}
							?>
						</tbody>
					</table>
		...
		<td valign='top' style='border: thin solid black; padding-top:20px'>
			</div>
</body>
